declare module "@vuelidate/core" {
  ...
  export interface ValidationRuleWithParams<
  ...
  type ValidationRuleCollection<T = unknown> = Record<
    string,
    ValidationRule<T>
  >;
  ...
  export interface RuleResultWithParams<P extends object = object>
    extends RuleResultWithoutParams {
    readonly $params: P;
  }
  ...
  > = R extends ValidationRuleWithParams<infer P>
    ? RuleResultWithParams<P>
    : RuleResultWithoutParams;

  type ExtractRulesResults<
    T,
    Vrules extends ValidationRuleCollection<T> | undefined
  > = {
    readonly [K in keyof Vrules]: Vrules[K] extends ValidationRule
      ? ExtractRuleResult<Vrules[K]>
      : undefined;
  };
  ...
  interface ChildValidations {
    ...
    readonly $getResultsForChild?: (
      key: string
    ) => (BaseValidation & ChildValidations) | undefined;
  }
  ...
  > = NestedValidations<Vargs, T> &
    BaseValidation<
      T,
      Vargs extends ValidationRuleCollection ? Vargs : undefined
    > &
    ChildValidations;

  type ExtractStateLeaf<
    Vrules extends ValidationRuleCollection
