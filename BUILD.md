...
vars first (see the env table below). Bind/workers come from the environment:
...
terminates at an optional Caddy reverse proxy (`deploy/hosted/`). The full
cold-start walkthrough — secrets, first login, HTTPS, backups, upgrades — is in
...
#### First administrator login (JWT)

Server mode requires a bearer token on every API route. Create the login once,
...
The browser SPA wires this token flow into a login screen (frontend auth); until
then the API is exercised with the bearer token directly as above.
